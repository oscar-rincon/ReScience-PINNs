...
def train_adam(model, x0, x1, u0, u1, lambda_1, lambda_2, dt, IRK_alpha, IRK_beta, num_iter=50_000):
    ...
    for i in range(1, num_iter + 1):
        ...
        if i % 100 == 0:
            torch.save(model.state_dict(), f'models_iters/Burgers_noisy_dtid_{iter}.pt')
...
def closure(model, optimizer, x0, x1, u0, u1, lambda_1, lambda_2, dt, IRK_alpha, IRK_beta):
    ...
    results.append([iter, loss.item(), error_lambda_1.item(), error_lambda_2.item()])
    if iter % 100 == 0:
        torch.save(model.state_dict(), f'models_iters/Burgers_dtid_noisy_{iter}.pt')
...
if __name__ == "__main__": 
    ...
    with open('training/Burgers_dtid_noisy_training_summary.txt', 'w') as file:
    ...
    np.savetxt("training/Burgers_dtid_noisy_training_data.csv", np.column_stack([results[:,0], results[:,1], error_lambda_1s, error_lambda_2s]), delimiter=",", header="Iter,Loss,ErrorLambda1,ErrorLambda2", comments="")
    ...
    # Save model state
    torch.save(model.state_dict(), 'Burgers_dtid_noisy.pt')        
