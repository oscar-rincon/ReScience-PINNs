...
if __name__ == "__main__":
    ...
    # Create directories for storing models and training logs if they don't exist
    ...
    device = torch.device('cpu') # Use 'torch.device('cuda' if torch.cuda.is_available() else 'cpu')' for GPU support
    print(f'Using device: {device}')

    # Define hyperparameters and model parameters
    q = 50  # IRK order
    skip = 120  # Time steps to skip for boundary condition
    N0 = 199  # Number of initial condition points
    N1 = 201  # Number of boundary condition points
    ...
    Exact = np.real(data['uu'])  # Exact solution
    idx_t = 40  # Time index for initial condition
    noise = 0.0  # Noise level (for data augmentation)
    ...
    dt = torch.tensor((t_star[idx_t+skip] - t_star[idx_t]).item()).to(device)  # Time step size
    ...
    results = []  # List to track training results         
    ...
    # Save model state
    torch.save(model.state_dict(), 'KdV_clean.pt') 
