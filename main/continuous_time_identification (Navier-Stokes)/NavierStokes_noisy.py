...
def train_adam(model, x_train_pt, y_train_pt, t_train_pt, u_train_pt, v_train_pt, num_iter=200_000):
    ...
    for i in range(1, num_iter + 1):
        ...
        iter += 1
        if iter % 100 == 0:
            torch.save(model.state_dict(), f'models/NS_noisy_{iter}.pt')
...
def closure(model, optimizer, x_train_pt, y_train_pt, t_train_pt, u_train_pt, v_train_pt, num_iter=50_000):
    ...
    results.append([iter, loss.item(), error_lambda_1.item(),error_lambda_2.item()])
    if iter % 100 == 0:
        torch.save(model.state_dict(), f'model/NS_noisy_{iter}.pt')
...
if __name__== "__main__":
    ...
    np.savetxt("training/NS_training_noisy.csv", results, delimiter=",", header="Iter,Loss,l1,l2", comments="")
    ...
    torch.save(model.state_dict(), f'training/NS_noisy.pt')
