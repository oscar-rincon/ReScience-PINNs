...
class ACNN(nn.Module):
    def __init__(self):
        ...
        self.linear_in = nn.Linear(1, 200, dtype=torch.float32)
        self.linear_out = nn.Linear(200, 101, dtype=torch.float32)
        self.layers = nn.ModuleList([nn.Linear(200, 200, dtype=torch.float32) for _ in range(5)])
        self.act = nn.Tanh()
...
if __name__ == "__main__":
    ...
    t = data['tt'].flatten()[:, None].astype(np.float32)
    x = data['x'].flatten()[:, None].astype(np.float32)
    Exact = np.real(data['uu']).T.astype(np.float32)
    idx_t0 = 20
    idx_t1 = 180
    dt = torch.from_numpy(t[idx_t1] - t[idx_t0]).to(torch.float32)
    noise_u0 = 0.0
    ...
    x1 = x1.to(device)
    dt = dt.to(device)
    IRK_weights = IRK_weights.to(device)
    u0 = u0.to(device)
